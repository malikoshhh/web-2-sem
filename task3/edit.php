...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$errors) {
        $updateStmt = $pdo->prepare("
            UPDATE users
            SET fullname = ?, phone = ?, email = ?, birthdate = ?, gender = ?, bio = ?, contract = ?
            WHERE id = ?
        ");
        $updateStmt->execute([
        ...
        $langInsert = $pdo->prepare("INSERT INTO user_languages (user_id, language) VALUES (?, ?)");
        foreach ($languages as $lang) {
            $langInsert->execute([$userId, $lang]);
        }
...
<div class="page-wrapper">
  ...
  <header class="form-header">
    <p class="eyebrow">Редактирование профиля</p>
    ...
    <div class="field-group">
      ...
      <div class="field-content">
        <select id="languages" name="languages[]" multiple>
          <option <?= in_array('Pascal', $userLanguages) ? 'selected' : '' ?>>Pascal</option>
          <option <?= in_array('C', $userLanguages) ? 'selected' : '' ?>>C</option>
          <option <?= in_array('C++', $userLanguages) ? 'selected' : '' ?>>C++</option>
          <option <?= in_array('JavaScript', $userLanguages) ? 'selected' : '' ?>>JavaScript</option>
          <option <?= in_array('PHP', $userLanguages) ? 'selected' : '' ?>>PHP</option>
          <option <?= in_array('Python', $userLanguages) ? 'selected' : '' ?>>Python</option>
          <option <?= in_array('Java', $userLanguages) ? 'selected' : '' ?>>Java</option>
          <option <?= in_array('Haskell', $userLanguages) ? 'selected' : '' ?>>Haskell</option>
          <option <?= in_array('Clojure', $userLanguages) ? 'selected' : '' ?>>Clojure</option>
          <option <?= in_array('Prolog', $userLanguages) ? 'selected' : '' ?>>Prolog</option>
          <option <?= in_array('Scala', $userLanguages) ? 'selected' : '' ?>>Scala</option>
          <option <?= in_array('Go', $userLanguages) ? 'selected' : '' ?>>Go</option>
        </select>
    ...
    <div class="field-group">
      ...
      <div class="field-content">
        <label class="checkbox-item">
          <input type="checkbox" name="contract" <?= $user['contract'] ? 'checked' : '' ?> required/>
